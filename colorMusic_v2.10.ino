...
#define REMOTE_TYPE 0       // 0 - без пульта

// ----- налаштування параметрів
#define KEEP_SETTINGS 1     // зберігати ВСІ налаштування в енергонезалежній пам'яті
#define KEEP_STATE 1		    // зберігати у пам'яті стан вкл/викл системи 
#define RESET_SETTINGS 0    // скидання налаштувань у EEPROM пам'яті 
#define SETTINGS_LOG 0      // виведення всіх налаштувань з EEPROM до порту під час запуску

// ----- налаштування стрічки
#define NUM_LEDS 60        // кількість світлодіодів 
#define CURRENT_LIMIT 3000  // ліміт по струму в міліамперах, автоматично керує яскравістю
byte BRIGHTNESS = 200;      // яскравість за замовчуванням (0 - 255)

// ----- пін підключення
#define SOUND_R A2         // аналоговый пін вхід аудіо, правий канал
#define SOUND_L A1         // аналоговый пін вхід аудіо, лівий канал
#define SOUND_R_FREQ A3    // аналоговый пін вхід аудіо для режима з частотами
#define BTN_PIN 3          // кнопка переключення режимів (PIN --- КНОПКА --- GND)

#if defined(__AVR_ATmega32U4__) // Піны для Arduino Pro Micro (смотри схему для Pro Micro на странице проекта!!!)
#define MLED_PIN 17             // пін светодиода режимов на ProMicro, т.к. обычный не выведен.
#define MLED_ON LOW
#define LED_PIN 9               // пін DI світлодіодної стрічки на ProMicro
#else                           // Піни для інших плат Arduino (за замовченням)
...
#define IR_PIN 2                // пін ІК приймача

// ----- налаштування кольорів веселки
float RAINBOW_STEP = 5.00;         // крок зміни кольору веселки

#define MODE 0                    // режим під час запуску
#define MAIN_LOOP 5               // період основного циклу малювання 
...
float averK = 0.006;
byte count;
float index = (float)255 / MAX_CH;   // коефіцієнт переводу для палітри
boolean lowFlag;
byte low_pass;
...
#if defined(__AVR_ATmega32U4__)   //Відключення світлодіодів на Pro Micro
  TXLED1;                           
  delay (1000);                     
#endif
  pinMode(MLED_PIN, OUTPUT);        //Режим піна для світлодіоду режиму на вихід
  digitalWrite(MLED_PIN, !MLED_ON); //Вимкнення світлодіода режиму
